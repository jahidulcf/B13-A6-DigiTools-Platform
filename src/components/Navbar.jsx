
const Navbar = ({cart}) => {
    return (
        <div className="shadow-sm">
            <div className="navbar bg-base-100 w-full container mx-auto gap-4">

                <div className="navbar-start">
                    <div className="dropdown">
                        <div tabIndex={0} role="button" className="btn btn-ghost md:hidden">
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"> <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h8m-8 6h16" /> </svg>
                        </div>
                        <ul
                            tabIndex="-1"
                            className="menu menu-sm dropdown-content bg-base-100 rounded-box z-1 mt-4 w-52 p-2 shadow">
                            <li><a href="#">Products</a></li>
                            <li><a href="#">Features</a></li>
                            <li><a href="#">Pricing</a></li>
                            <li><a href="#">Testimonals</a></li>
                            <li><a href="#">FAQ</a></li>
                        </ul>
                    </div>
                    <a className="btn btn-ghost text-xl">
                        <span className="bg-linear-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">
                            DigiTools
                        </span>
                    </a>
                </div>

                <div className="navbar-center justify-center hidden md:flex">
                    <ul className="menu menu-horizontal px-1 text-gray-900">
                        <li><a href="#">Products</a></li>
                        <li><a href="#">Features</a></li>
                        <li><a href="#">Pricing</a></li>
                        <li><a href="#">Testimonals</a></li>
                        <li><a href="#">FAQ</a></li>
                    </ul>
                </div>

                <div className="navbar-end flex gap-2">
                    <div tabIndex={0} role="button" className="btn btn-ghost btn-circle">
                        <div className="indicator">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"> <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" /> </svg>
                        {cart.length > 0 && <span className="badge badge-sm indicator-item">{cart.length}</span>}
                        </div>
                    </div>
                    <a href="#" className="btn btn-ghost rounded-full">Login</a>
                    <a className="btn rounded-full bg-linear-to-r from-indigo-600 to-purple-600 text-white hidden md:inline-flex">Get started</a>
                </div>
            </div>
        </div>
    );
};

export default Navbar;